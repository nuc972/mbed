...
/* NS replacement for nu_idle_veneer/nu_powerdown_veneer due to acquiring NS lock
 * when interrupt is disabled. */
static int32_t ns_repl_wfi(uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
    __WFI();
}
...
void nu_idle_s(void)
{
    nu_tfm_ns_lock_dispatch2(nu_idle_veneer, 0, 0, 0, 0, ns_repl_wfi);
}

void nu_powerdown_s(void)
{
    nu_tfm_ns_lock_dispatch2(nu_powerdown_veneer, 0, 0, 0, 0, ns_repl_wfi);
}
